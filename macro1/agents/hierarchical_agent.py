...
from macro1.schema.config import SubAgentConfig, HierarchicalAgentConfig
...
class HierarchicalAgent(Agent):
    def __init__(self, config_path: str = None, **kwargs):
        ...
        if config_path is not None:
            self.config = HierarchicalAgentConfig.from_yaml(config_path)
        else:
            self.config = HierarchicalAgentConfig(**kwargs)
        ...
        if self.status == AgentStatus.FINISHED and self.task_data.sub_tasks is not None:
            ...
            if self.task_data.current_sub_task_idx + 1 < len(self.task_data.sub_tasks):
                if self.task_data.task_type == 'A':
                    ...
                    new_trajectory: List[Macro1StepData] = []
                    new_episodedata = Macro1EpisodeData(goal=new_goal, num_steps=0, trajectory=new_trajectory)
