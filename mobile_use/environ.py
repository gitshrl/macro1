...
class Environment:
    ...
    def get_state(self):
        try:
            # Use screencap directly for correct resolution (adbutils screenshot may resize)
            import subprocess, io
            result = subprocess.run(
                ['adb', '-s', self._d.serial, 'exec-out', 'screencap', '-p'],
                capture_output=True, timeout=10
            )
            from PIL import Image
            pixels = Image.open(io.BytesIO(result.stdout))
        except Exception as e:
